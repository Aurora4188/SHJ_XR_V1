...
function updateThree() {
  ...
  //updateFlyingRoute();
  t++;
  let radDist = map(sin(frame * 0.01), -1, 1, 30, 80);
  // console.log("creature", creature);

  //rotate the creature group
  //creature.rotation.y = sin(frame * 0.01) * PI;

  //console.log("BP1", bodyPart01);
  if (bodyPart01) {
    console.log("!!!!", bodyPart01);
    /*
    if (bodyPart01.position !== undefined) {
      creature.position.x += 1;
      creature.position.y += 1;
      // console.log("creature", creature.position.x);
  
  
      // bodyPart01.Promise.children[1].position.x = cos(t);
      // bodyPart01.position.z = sin(frame * 0.01) * radDist;
    }
    */
  }
  // bodyPart01.position.x += 1;
  // bodyPart02.position.x += 1;
  // creature.position.x += 1
  // creature.position.y += 1; // changing the x, y value for position
  // console.log("creature", creature.position.x);

  //here!
  // if (bodyPart01) bodyPart01.rotateY(frame * -0.01);
  // if (bodyPart02) bodyPart02.rotateY(frame * 0.01);
  // if (bodyPart03) bodyPart03.rotateY(frame * 0.02);
  // if (bodyPart04) bodyPart04.rotateY(frame * -0.02);
...
/*
function loadBodyGLTF(filepath) {
  return new Promise((resolve, reject) => {
    ...
    loader.load(
      ...
      (gltfData) => {
        ...
        creature.add(gltfData.scene.children[0]);
        resolve(gltfData.scene.children[0]); // the first child of the loaded scene
      },
      undefined, // You can handle progress here if you want
      (error) => {
...
class Spring {
  ...
  updateMesh() {
    ...
    this.mesh.scale.x = direction.length(); //?
...
class Connection {
  constructor(x, y, z, r, modelP) {
    ...
    this.modelP = modelP;//here is the model loaded
    this.model = null;
    //
    this.damping = 0.94; // -0.03%
  ...
  update() {
    ...
    if (!this.vel.equals(new THREE.Vector3(0, 0, 0))) {
      const direction = this.vel.copy().normalize();
      const quaternion = new THREE.Quaternion();
      quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), direction); // Assuming up vector is Y-axis
...
function setupSpringMeshes() {
  // create connections
  // console.log("Setting up spring meshes");
  let bodyM01, bodyM02, bodyM03, bodyM04;

  //bodyM01P = loadGLTF('assets/YYFish3.gltf');
  // bodyM02 = getTestModel1();

  // bodyPart01 = loadBodyGLTF('assets/bodyPart01-head.gltf').then(model1 => {
  //   creature.add(model1);
  ...
  for (let i = 0; i < bodyPartGLTFpaths.length; i++) {
    ...
    loadBodyGLTF(filepath, function (model) {
      bodyParts[i] = model;
      creature.add(model);
      connections.push(new Connection(0, 80, 0, 5, model));
      model.scale.set(BODYPART_SCALE, BODYPART_SCALE, BODYPART_SCALE);
    });
  }
  loadBodyGLTF('assets/bodyPart01-head.gltf', function (model) {
    bodyPart01 = model;
    creature.add(model);
    connections.push(new Connection(0, 80, 0, 5, bodyPart01));
    model.scale.set(BODYPART_SCALE, BODYPART_SCALE, BODYPART_SCALE);
  });


  loadBodyGLTF('assets/bodyPart02.gltf', function (model) {
    bodyPart02 = model;
    creature.add(model);
    connections.push(new Connection(2, 80, 0, 5, bodyPart02));
    model.scale.set(BODYPART_SCALE, BODYPART_SCALE, BODYPART_SCALE);
  });

  loadBodyGLTF('assets/bodyPart03.gltf', function (model) {
    bodyPart03 = model;
    creature.add(model);
    connections.push(new Connection(4, 80, 0, 5, bodyPart03));
    model.scale.set(BODYPART_SCALE, BODYPART_SCALE, BODYPART_SCALE);
  });

  loadBodyGLTF('assets/bodyPart04_Tail.gltf', function (model) {
    bodyPart04 = model;
    creature.add(model);
    connections.push(new Connection(6, 100, 0, 5, bodyPart04));
    model.scale.set(BODYPART_SCALE, BODYPART_SCALE, BODYPART_SCALE);
  });

  // connect the connections with springs
  /*
  for (let i = 0; i < connections.length - 1; i++) {
    let a = connections[i];
    let b = connections[i + 1];
    let len = 0; //connection distance
    let s = new Spring(a, b, len);
    springs.push(s);
  }
  */
}
...
function updateSpringMeshes() {
  ...
  let radDist = map(sin(frame * 0.01), -1, 1, 30, 80);
  let x = cos(frame * 0.01) * radDist;
  let y = sin(frame * 0.01) * radDist;
  connections[0].pos.x = x;
  connections[0].pos.y = y + 100;
...
function updateSwimMotion() {
  swimOffset += 0.05; // Adjust this value for speed

  // a wave-like motion to each part
  for (let i = 0; i < connections.length; i++) {
    let phase = swimOffset + i * 0.5; // Adjust the 0.5 for phase difference between parts
    connections[i].pos.y += Math.sin(phase) * 2; // Vertical movement
    connections[i].pos.x += Math.cos(phase) * 2; // Horizontal movement
...
//   for (let i = 0; i < connections.length; i++) {
//     let phase = swimOffset + i * 0.5;
//     connections[i].pos.y += Math.sin(phase) * 2;
//     connections[i].pos.x += Math.cos(phase) * 2;

//     // console.log(`Connection ${i} - New position: x=${connections[i].pos.x}, y=${connections[i].pos.y}`);

//     // Ensure this updates the actual model position
//     connections[i].update();
//   }
// }
